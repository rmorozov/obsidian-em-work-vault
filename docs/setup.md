...
2. Enable your existing `periodic-para` (LifeOS), Dataview, and Templater plugins in the local copy. Do not commit the copied `.obsidian` directory or plugin data. Configure Templater's folder to `Templates` if applying these standalone drafts through Templater; LifeOS may use its own configured template paths.
3. Let LifeOS create daily and weekly notes in a throwaway copy. Inspect its settings for daily/weekly folder and file formats, template paths, and Daily Record heading. Your vault is based on the example project, whose source templates live in `0. PeriodicNotes/Templates/`. Compare those templates with these drafts and selectively merge sections rather than replacing the originals.
